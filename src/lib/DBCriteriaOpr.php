...
class DBCriteriaOpr {
    ...
    public function isNull(){
        $this->criteria->queryWith('IS NULL ');
        return $this->criteria;
    }

    public function isNotNull(){
        $this->criteria->queryWith('IS NOT NULL ');
        return $this->criteria;
    }
    ...
    public function in(array $cols,array $params){
        $this->criteria->queryWith("IN ".$this->formatArray($cols));
        $this->criteria->addParams($params);
        return $this->criteria;
    }

    public function notIn(array $cols,array $params){
        $this->criteria->queryWith("NOT IN ".$this->formatArray($cols));
        $this->criteria->addParams($params);
        return $this->criteria;
    }
    ...
    private function buildBasicOperand(string $col,$param=null,$opr){
        if($param===null) $this->criteria->queryWith("$opr $col ");
        else {
            $this->criteria->queryWith("$opr ? ");
